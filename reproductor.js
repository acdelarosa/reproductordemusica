...
mostrarBusqueda(filtroDeCanciones) {
   ...
   play = function() {
    ...
    if(this.actualSong !== undefined){
        this.audio.src = "canciones/" + this.actualSong.urlSong;
        this.audio.play();
    }
    else{
        let id;
        switch (this.actualPlaylist){
            case 'favoritos':id= document.getElementById("play");
            case 'playlist':id= document.getElementById("play");
            case 'busqueda':id= document.getElementById("play");
               break;
...
cambiarPortada = function() {
    ...
    cover.src = "album/"+this.actualSong.cover;
}
